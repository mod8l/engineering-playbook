# Engineering Playbook

[![License: CC BY 4.0](https://img.shields.io/badge/License-CC%20BY%204.0-lightgrey.svg)](https://creativecommons.org/licenses/by/4.0/)
[![Markdown Lint](https://github.com/mod8l/engineering-playbook/actions/workflows/markdown-lint.yml/badge.svg)](https://github.com/mod8l/engineering-playbook/actions/workflows/markdown-lint.yml)

A public, employer-agnostic playbook for R&D leadership: how I hire, review code, respond to incidents, adopt AI responsibly, and scale startups to production.

I use these templates when I advise founders or join a company as VP R&D. The goal is not to ship a rulebook; it is to give teams a starting point they can adapt quickly so they spend less time arguing about process and more time shipping reliable systems.

**Related work:** [`ml-inference-starter`](../ml-inference-starter) for a reference serving architecture, [`ml-security-checklist`](../ml-security-checklist) for production readiness and security, [`platform-scripts`](../platform-scripts) for reusable tooling, and [`publications-and-talks`](../publications-and-talks) for essays and talks.

This repo is a living set of opinions and templates, not a rulebook. Every organization has different constraints, so treat each page as a starting point for conversation rather than a mandate. I have written it in first-person voice where it felt natural, because leadership practices are ultimately personal: they reflect what I have seen work, what I have seen fail, and what I am still learning.

## Table of Contents

- [R&D Principles](r-and-d-principles.md)
- [Hiring](hiring/)
  - [Role Template](hiring/role-template.md)
  - [Interview Loop](hiring/interview-loop.md)
  - [Take-Home Exercise](hiring/take-home-exercise.md)
- [Code Review](code-review/)
  - [PR Template](code-review/pr-template.md)
  - [Review Checklist](code-review/review-checklist.md)
  - [Examples](code-review/examples/)
- [Incident Response](incident-response/)
  - [Severity Levels](incident-response/severity-levels.md)
  - [Runbook Template](incident-response/runbook-template.md)
  - [Postmortem Template](incident-response/postmortem-template.md)
- [AI Adoption](ai-adoption/)
  - [Evaluation Framework](ai-adoption/evaluation-framework.md)
  - [Responsible AI Checklist](ai-adoption/responsible-ai-checklist.md)
  - [Experiment Log Template](ai-adoption/experiment-log-template.md)
- [Leadership](leadership/)
  - [R&D Org Design](leadership/rd-org-design.md)
  - [Startup Scaling Playbook](leadership/startup-scaling-playbook.md)
  - [Board Communications](leadership/board-communications.md)
  - [Technical Due Diligence](leadership/technical-due-diligence.md)
  - [R&D Metrics](leadership/rd-metrics.md)
  - [Platform Economics](leadership/platform-economics.md)
  - [Essays](leadership/essays/)
    - [First Six Months of ML at a Startup](leadership/essays/first-six-months-of-ml-at-a-startup.md)
    - [Why Your AI Demo Is Not a Product](leadership/essays/why-your-ai-demo-is-not-a-product.md)
    - [Hiring for Shipping, Not Just Research](leadership/essays/hiring-for-shipping-not-just-research.md)

## How to Use This Repo

1. **Read the principles first.** The [R&D principles](r-and-d-principles.md) are the spine; everything else flows from them.
2. **Pick a practice area.** Each section has a `README.md` that explains intent and links to concrete templates.
3. **Fork the templates.** Replace bracketed placeholders and adapt checklists to your context.
4. **Challenge the content.** If a template does not fit your team, open an issue or propose a change. Good process is discovered, not decreed.

## Contributing

I welcome corrections, clarifications, and new perspectives. Because this repo is intentionally non-proprietary, please do not submit internal company documents, proprietary data, or employer-specific anecdotes. Keep examples fictional and employer-agnostic.

If you adapt this material elsewhere, a credit link back to this repo is appreciated but not required under the CC-BY-4.0 license.
